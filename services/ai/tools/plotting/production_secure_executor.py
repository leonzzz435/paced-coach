...
_ENABLE_UNSAFE_EXECUTION_ENV_VAR = "ENABLE_UNSAFE_PLOTTING_EXECUTION"
...
def run_plot_code_get_html(code: str, timeout_s: int = 6):
    raw_flag = os.getenv(_ENABLE_UNSAFE_EXECUTION_ENV_VAR, "").strip().lower()
    execution_enabled = raw_flag in {"1", "true", "yes", "on"}
    if not execution_enabled:
        error_msg = (
            "Plot code execution is disabled by default for security. "
            f"Set {_ENABLE_UNSAFE_EXECUTION_ENV_VAR}=true only in trusted, isolated environments."
        )
        logger.warning(
            "Rejected plotting code execution because %s is not enabled",
            _ENABLE_UNSAFE_EXECUTION_ENV_VAR,
        )
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        try:
            ...
            return {"ok": False, "error": f"Execution failed: {exc!s}"}
