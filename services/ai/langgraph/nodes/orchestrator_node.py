...
class ConsoleInteractionProvider:
    def collect_answers(self, questions: list[dict], stage_name: str) -> list[dict]:
        ...
        for i, qa in enumerate(questions, 1):
            ...
            logger.info(
                "User answered %s question %s (answer_chars=%s)",
                agent_name,
                i,
                len(user_answer),
            )
...
class MasterOrchestrator:
    STAGES = {
        ...
        "weekly_planning": {
            ...
            "next_node": ["season_formatter", "weekly_formatter"],
    ...
    def __call__(self, state: TrainingAnalysisState) -> Command:
        ...
        def _resolve_next_node():
            ...
            needs_season_formatting = bool(state.get("season_plan_needs_formatting", True)) or not bool(
                state.get("season_plan_blocks")
            )
            next_nodes: list[str] = ["weekly_formatter"]
            if needs_season_formatting:
                next_nodes.insert(0, "season_formatter")
            logger.info(
                "MasterOrchestrator: Weekly planning next=%s (season_formatter=%s)",
                next_nodes,
                needs_season_formatting,
            )
            return next_nodes
        ...
        if not all_questions:
            if stage == "analysis":
                ...
                return Command(goto=_resolve_next_node())
