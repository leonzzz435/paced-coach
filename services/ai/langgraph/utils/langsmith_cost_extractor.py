...
class LangSmithCostExtractor:
    ...
    def extract_workflow_costs_by_trace(self, trace_id: str, execution_time: float = 0.0) -> WorkflowCostSummary:
        ...
        try:
            all_runs = list(
                self.client.list_runs(trace=trace_id, select=["id", "name", "run_type", "total_cost", "total_tokens"])
            )
    ...
    def extract_run_costs(self, run_id: str) -> dict[str, Any]:
        try:
            ...
            return {
                ...
                "total_input_tokens": workflow_summary.total_input_tokens,
                "total_output_tokens": workflow_summary.total_output_tokens,
                "total_web_searches": workflow_summary.total_web_searches,
