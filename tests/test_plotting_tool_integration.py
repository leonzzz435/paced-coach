...
class TestPlottingToolIntegration:
    ...
    @pytest.mark.asyncio
    async def test_tool_invocation_denied_by_default(self, monkeypatch):
        monkeypatch.delenv("ENABLE_UNSAFE_PLOTTING_EXECUTION", raising=False)
...
"""
        ...
        assert "disabled by default for security" in result["error"]

    @pytest.mark.asyncio
    async def test_tool_invocation(self, monkeypatch):
        monkeypatch.setenv("ENABLE_UNSAFE_PLOTTING_EXECUTION", "true")
