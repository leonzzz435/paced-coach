...
from services.whoop.oauth import compute_expires_at
from services.whoop.oauth import refresh_tokens as _refresh_tokens_sync
...
async def ensure_valid_access_token(db, *, user_id) -> str:
    ...
    # Lock to avoid concurrent refresh requests invalidating each other.
    row = await db.execute(
        select(WhoopCredentials).where(WhoopCredentials.user_id == user_id).with_for_update()
    )
    creds = row.scalar_one_or_none()
    if creds is None:
        raise HTTPException(status_code=404, detail="WHOOP is not connected for this account.")
    ...
    if not creds.encrypted_refresh_token:
        raise HTTPException(status_code=409, detail="WHOOP refresh token is missing. Please reconnect WHOOP.")
    ...
    try:
        ...
        # Invalid grant or unauthorized: force reconnect.
        if status_code in {400, 401}:
            logger.info("Whoop token refresh rejected (status=%s); deleting stored credentials", status_code)
            ...
            await db.commit()
            raise HTTPException(status_code=401, detail="WHOOP connection expired. Please reconnect WHOOP.") from exc
        raise HTTPException(status_code=502, detail="WHOOP token refresh failed. Please try again.") from exc
    except httpx.HTTPError as exc:
        raise HTTPException(status_code=502, detail="WHOOP token refresh failed. Please try again.") from exc

    access_token = payload.get("access_token")
    if not isinstance(access_token, str) or not access_token.strip():
        raise HTTPException(status_code=502, detail="WHOOP token refresh returned an invalid access token.")

    new_refresh = payload.get("refresh_token")
    expires_at = compute_expires_at(now=now, expires_in=payload.get("expires_in"))
    scope = payload.get("scope") or creds.scope or ""
